...
<head>
  ...
  <style>
    *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
    ...
    .best-match .prob-hint {
      font-size: 0.78rem;
      color: #718096;
      margin-bottom: 10px;
    }
    .best-match .citation-text {
      font-size: 0.95rem;
      color: #2d3748;
      margin-bottom: 12px;
      line-height: 1.6;
    ...
    .best-match .meta a:hover { text-decoration: underline; }
    ...
    .alt-meta a { color: #4299e1; text-decoration: none; }
...
<body>
  <header>
    ...
    <p>Введите библиографическое описание — инструмент найдёт наиболее вероятную статью через Crossref и OpenAlex.</p>
  </header>

  <div class="card">
    <form id="searchForm" method="post" action="/match">
      <div class="search-row">
        <textarea name="query" id="queryInput" placeholder="Например: Deep learning LeCun Bengio Hinton Nature 2015">{{ query }}</textarea>
    ...
    <div class="results">
      <div class="best-match">
        ...
        <div class="prob">{{ "%.1f"|format(best.confidence * 100) }}%</div>
        <div class="prob-hint">относительная уверенность среди найденных кандидатов</div>
        <div class="citation-text">{{ best.formatted_citation }}</div>
        <div class="meta">
          {% if best.doi %}
            DOI: <a href="https://doi.org/{{ best.doi }}" target="_blank">{{ best.doi }}</a>
          {% endif %}
      ...
      <ul class="alt-list">
        ...
        <li class="alt-item">
          <div class="alt-prob">{{ "%.1f"|format(alt.confidence * 100) }}%</div>
          <div class="alt-body">
            <div class="alt-title">
              {{ alt.title }}
              <span class="source-tag source-{{ alt.source }}">{{ alt.source }}</span>
            </div>
            <div class="alt-meta">
              {% if alt.doi %}<a href="https://doi.org/{{ alt.doi }}" target="_blank">{{ alt.doi }}</a>{% endif %}
              {% if alt.authors %} · {{ alt.authors }}{% endif %}
              {% if alt.year %} · {{ alt.year }}{% endif %}
            </div>
          </div>
        </li>
        {% endfor %}
  ...
  <footer>Данные: Crossref API &amp; OpenAlex API · Citation Matcher v0.1</footer>
...
async def match(query: str = Form(...)) -> HTMLResponse:
    ...
    return _render(
        ...
        alternatives=result.alternatives[:9],
    )
